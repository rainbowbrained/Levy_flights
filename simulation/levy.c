/*
levy_batch_infinite_viswanathan.c

Goal:
- Correct Viswanathan-style Lévy search simulation (in 2D PPP world) where:
  1) A flight is a straight motion with planned length L drawn from truncated power-law.
  2) If ANY target is detected within radius rv during the flight, the searcher
     goes to that target and the flight TERMINATES immediately.
  3) The next flight starts from the target position with a new (theta, L).
  4) No “dead-remainder” travel after capture.

- Adds/ensures the metric:
    normalized_rate_visits_global = lambda * eta_visits_global
  where:
    eta_visits_global = (total number of visits) / (total distance),
    lambda = 1/(2*rho*rv) (mean free path along a line).

Build:
  gcc -O3 -std=c11 -march=native -ffast-math -o levy_batch_infinite levy_batch_infinite_viswanathan.c -lm

Notes:
- This is a 2D simulation. Fig.2a,b in Viswanathan (1999) are 1D; Fig.2c is 2D.
- To compare to Fig.2-style curves, you typically plot:
    normalized_rate_visits_global vs mu (for different lambda).
*/
...
static void run_one_series(uint64_t series_seed, const Params* P, double mu, RunMetrics* metrics){
    ...
    // ignore last target until (almost) outside its detection disk
    ...
    for(int k=0; k<P->flights; ++k){
        ...
        while(flight_left > 0.0){
            ...
            if(find_first_detection_in_segment(&cm, pos, dir, seg_len, P->rv,
                ...
                // CRITICAL: Correct Viswanathan rule:
                // terminate flight immediately after capture; do NOT travel remaining planned length.
                (void)captured;
                break;
            } else {
