/*
levy_no_interrupt.c — NO FLIGHT INTERRUPTION

Rule:
  The searcher flies along a straight pre-planned trajectory of length L.
  If a target is detected within rv during the flight, the visit is COUNTED
  but the flight is NOT interrupted: the searcher continues along the SAME
  straight line until the full planned length L is exhausted.
  Multiple targets may be detected within a single flight.

  After the flight is complete, a new direction theta and length L are sampled.

  Supports destructive (--remove-prob 1) and non-destructive (--remove-prob 0) modes.

Build:
  gcc -O3 -std=c11 -march=native -o levy_no_interrupt levy_no_interrupt.c -lm
*/
    ...
    for(int k=0; k<P->flights; ++k){
        ...
        while(flight_left > 0.0){
            ...
            if(find_first_detection_in_segment(&cm, pos, dir, seg_len, P->rv,
                                               ...
                                               &s_hit, &det_point)) {
                // Target detected at s_hit along segment.
                // Agent does NOT deviate from trajectory; visit is counted at detection point.
                TargetRef ref;
                if(find_nearest_visible_target(&cm, det_point, P->rv, last_id, ignore_last_within, &ref)){
                    Target tgt = ref.chunk->targets[ref.idx];

                    // Count VISIT at detection time (total_time + s_hit)
                    metrics->visits++;
                    record_visit_time(metrics, metrics->total_time + s_hit);
            ...
            // ALWAYS move to segment end — flight is never interrupted
            Vec2 endp = add2(pos, mul2(dir, seg_len));
            move_to(metrics, &pos, endp);
            flight_left -= seg_len;
        }
        // Flight complete → new (theta, L)
    }
