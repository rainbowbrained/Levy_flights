/*
levy_teleport.c — FLIGHT INTERRUPT + TELEPORT lc FROM TARGET

Rule:
  The searcher flies along a straight trajectory of length L.
  If a target is detected within rv, the searcher:
    1) Moves to the target center (visit is counted, distance is added).
    2) Flight TERMINATES immediately.
    3) Searcher TELEPORTS to a random point at distance lc from the target.
       (The teleport distance lc IS counted in total_distance.)
    4) A new flight begins from the teleported position (new theta, new L).

  If lc = 0, the behavior is identical to levy_viswanathan.c.

  Supports destructive (--remove-prob 1) and non-destructive (--remove-prob 0) modes.

Build:
  gcc -O3 -std=c11 -march=native -o levy_teleport levy_teleport.c -lm
*/
...
typedef struct {
    ...
    // teleport
    double lc;           // teleport distance from target after capture (0 = no teleport)
    ...
    for(int k=0; k<P->flights; ++k){
        ...
        while(flight_left > 0.0){
            ...
            if(find_first_detection_in_segment(&cm, pos, dir, seg_len, P->rv,
                ...
                // Find nearest target, move to its center
                TargetRef ref;
                ...
                // TELEPORT: move to random point at distance lc from target
                if(P->lc > 0.0){
                    double tp_theta = rng_angle(&rng);
                    Vec2 tp_dir = (Vec2){cos(tp_theta), sin(tp_theta)};
                    Vec2 tp_pos = add2(tgt_pos, mul2(tp_dir, P->lc));
                    move_to(metrics, &pos, tp_pos);
                }
...
"\n"
"Teleport:\n"
"  --lc <f>               teleport distance from target after capture (default 0)\n"
"\n"
"Scoring (optional):\n"
...
int main(int argc, char** argv){
    ...
    P.lc = get_argd(argc,argv,"--lc",P.lc);
    ...
    if(!append_mode){
        fprintf(f,
            "rho,rv,mu,lmin,lmax,dx,chunk_size,remove_prob,lc,flights,seed,reps,lambda,"
    ...
    for(;; ai++){
        ...
        fprintf(f,
            "%.6g,%.6g,%.6g,%.6g,%.6g,%.6g,%.6g,%.6g,%.6g,%d,%d,%d,%.12g,"
            ...
            "%.6f,%.6f,%.6f,%.6f,"
            "%.6g,%.6g,%.6g,%s\n",
            P.rho, P.rv, mu, P.lmin, P.lmax, P.dx, P.chunk_size, P.remove_prob, P.lc, P.flights, P.seed, P.reps, lambda,
